...
void Player::Jump(const float _delta_time, std::unordered_map<sf::Keyboard::Key, bool>* pressed_input, std::vector<GameObject*>* gameObjects, DoubleJump* capacityJump) {
	if (can_jump && !is_jumping) {
		for (const auto& input : *pressed_input) {
 			if (input.first == 57 && input.second == true) {
				...
				else if (capacityJump != nullptr)
				{
					if (capacityJump->GetJump())
					{
						capacityJump->SetJump(false);
...
void Player::Update(const float _delta_time, std::unordered_map<sf::Keyboard::Key, bool>* pressed_input) {
	...
	if (!setCapacityPlayer)
	{
		if (currentCapacity->GetName() == "DoubleJump")
		{
			capacityJump = GetOwner()->GetCapacity<DoubleJump>();
	...
	Jump(_delta_time, pressed_input, scene->GetGameObjects(), capacityJump);
	...
	if (GetOwner()->GetPosition().GetY() + sizePlayer <= sizeWindow.y && GetOwner()->GetComponent<SquareCollider>()->GetCanMoving()["down"]) {
		...
		if (capacityJump != nullptr)
		{
			if(!capacityJump->GetJump())
			{
				can_jump = false;
			}
			else if (capacityJump->GetJump())
			{
				can_jump = true;
			}
		}
